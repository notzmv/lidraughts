...
import lidraughts.socket.{ SocketMember, DirectSocketMember, RemoteSocketMember }
import lidraughts.socket.Socket.{ Sri, Sris }
...
private[lobby] object LobbySocketMember {
  ...
  def apply(bus: lidraughts.common.Bus, user: Option[User], blocking: Set[String], sri: Sri, mobile: Boolean): LobbyRemoteSocketMember =
...
private[lobby] case class JoinHook(sri: Sri, hook: Hook, game: Game, creatorColor: draughts.Color)
