...
private object UciToPdn {
  ...
  private def mk(sit: Situation, ucis: List[Uci], moves: List[Move], ambs: List[(Uci, String)]): Option[(Situation, List[Move])] = {
  ...
  def apply(situation: Situation, variation: List[Uci]): Valid[List[PdnMove]] = for {
    moves ← mk(situation, variation, Nil, Nil) toValid "Invalid variation " + variation
