    ...
    historyApi get user.id map2 { (history: History) =>
      lidraughts.common.String.html.safeJsonValue {
        Json.toJson {
          ...
          List(Bullet, Blitz, Rapid, Classical, Correspondence, Frisian, Frysk, Antidraughts, Breakthrough, Russian, Brazilian, Puzzle, PuzzleFrisian, PuzzleRussian, UltraBullet) map { pt =>
            Json.obj(
              "name" -> pt.name,
