...
object BSONHandlers {
  ...
  implicit val battleBSONHandler = lidraughts.db.BSON.LoggingHandler(logger)(Macros.handler[TeamBattle])
  ...
  implicit val tournamentHandler = new BSON[Tournament] {
    ...
    def writes(w: BSON.Writer, o: Tournament) = $doc(
      ...
      "fen" -> o.position.some.filterNot(_.initialVariant(o.variant)).map(_.fen),
      "table" -> o.openingTable.map(_.key),
