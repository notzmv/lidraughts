        ...
        PostRepo findDuplicate post flatMap {
          ...
          case _ =>
            env.postColl.insert(post) >>
              ...
              ctx.userId.?? { userId =>
                shutup ! {
                  if (post.isTeam) lidraughts.hub.actorApi.shutup.RecordTeamForumMessage(userId, post.text)
                  else lidraughts.hub.actorApi.shutup.RecordPublicForumMessage(userId, post.text)
                }
              } >>- {
      ...
      post.fold[Fu[Post]](fufail("Post no longer exists.")) {
        ...
        case (_, post) =>
          ...
          (newPost.text != post.text).?? {
            env.postColl.update($id(post.id), newPost).void
          } inject newPost
      }
    }
