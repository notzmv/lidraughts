      ...
      case (slug, lang) =>
        ...
        env.postColl.insert(post) >>
          ...
          ctx.userId.?? { userId =>
            ...
            shutup ! {
              if (post.isTeam) lidraughts.hub.actorApi.shutup.RecordTeamForumMessage(userId, text)
              else lidraughts.hub.actorApi.shutup.RecordPublicForumMessage(userId, text)
            }
          } >>- {
