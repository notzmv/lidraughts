...
import lidraughts.common.base.StringUtils.escapeHtmlRaw
...
final object RawHtml {
  ...
  private[this] val urlPattern = (
    ...
    """(?<![/@.-].)(?:\w{1,15}+\.){1,3}(?>com|org|edu))""" + // "lidraughts.org", etc
    """([/?#][-–—\w/.~!$&'()*+,;=:#?@%]{0,300}+)?""" + // path, params
    """(?![\w/~$&*+=#@%])""" // neg lookahead
  ).r.pattern
  ...
  private[this] final val DOMAIN = "lidraughts.org"
  ...
  // No: contact@lidraughts.org, @1, http://example.com/@happy0, @lidraughts.org
