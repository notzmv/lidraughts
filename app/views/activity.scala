...
object activity {
  ...
  private def renderPosts(posts: Map[lidraughts.forum.Topic, List[lidraughts.forum.Post]])(implicit ctx: Context) =
    entryTag(
      iconTag("d"),
      div(
        posts.toSeq.map {
  ...
  private def renderCorresMoves(nb: Int, povs: List[lidraughts.game.LightPov])(implicit ctx: Context) =
    entryTag(
      iconTag(";"),
      div(
  ...
  private def renderCorresEnds(score: Score, povs: List[lidraughts.game.LightPov])(implicit ctx: Context) =
    entryTag(
      iconTag(";"),
      div(
  ...
  private def renderStudies(studies: List[lidraughts.study.Study.IdName])(implicit ctx: Context) =
    entryTag(
      iconTag("4"),
      div(
  ...
  private def renderTours(tours: lidraughts.activity.ActivityView.Tours)(implicit ctx: Context) =
    entryTag(
      iconTag("g"),
      div(
