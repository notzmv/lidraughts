...
object timeline {
  ...
  def entries(entries: Vector[lidraughts.timeline.Entry])(implicit ctx: Context) =
    div(cls := "entries")(
  ...
  private def entry(e: lidraughts.timeline.Entry)(implicit ctx: Context) = frag(
    e.decode.map[Frag] {
      ...
      case GameEnd(playerId, opponent, win, perfKey) => lidraughts.rating.PerfType(perfKey) map { perf =>
        (win match {
      ...
      case PlanStart(userId) => a(href := routes.Plan.index)(
      ...
      case StreamStart(id, name) => a(cls := "text", dataIcon := "", href := routes.Streamer.show(id))(trans.xStartedStreaming(name))
    },
    " ",
