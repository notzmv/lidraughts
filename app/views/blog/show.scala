...
object show {
  ...
  def apply(doc: io.prismic.Document)(implicit ctx: Context, prismic: lidraughts.blog.BlogApi.Context) =
        ...
        main(cls := "page-menu page-small")(
          ...
          div(cls := s"blog page-menu__content box post ${~doc.getText("blog.cssClasses")}")(
            ...
            div(cls := "body embed_analyse")(
              doc.getHtml("blog.body", prismic.linkResolver).map(lidraughts.blog.Youtube.fixStartTimes).map(lidraughts.blog.ProtocolFix.remove).map(raw)
            ),
            NotForKids {
              div(cls := "footer")(
