...
object index {
  ...
  def apply(tokens: List[lidraughts.oauth.AccessToken])(implicit ctx: Context) = {
    ...
    views.html.account.layout(title = title, active = "oauth.token")(
      div(cls := "account oauth box")(
        ...
        p(cls := "box__pad")(
          "You can make OAuth requests without going through the authorization code flow.", br, br,
          "Instead, ",
          ...
          "Here's a ",
          a(href := "https://github.com/roepstoep/lidraughts-api/tree/master/example/oauth-personal-token")("personal token app example"),
          " and the ",
        ...
        tokens.headOption.filter(_.isBrandNew).map { token =>
          div(cls := "box__pad brand")(
            ...
            div(
              p("Make sure to copy your new personal access token now.", br, "You won’t be able to see it again!"),
              code(token.id.value)
            )
          )
        },
        table(cls := "slist slist-pad")(
          tokens.map { t =>
            tr(
              td(
                strong(t.description | "Unnamed"), br,
