...
object teamBattle {
  ...
  def teamInfo(tour: Tournament, team: lidraughts.team.Team.Mini, info: TeamBattle.TeamInfo)(implicit ctx: Context) =
    views.html.base.layout(
      title = s"${tour.fullName} • ${team.name}",
      moreCss = cssTag("tournament.show.team-battle")
    )(
        main(cls := "box")(
          h1(
            a(href := routes.Tournament.battleTeams(tour.id))(tour.fullName),
            " • ",
          ...
          table(cls := "slist slist-pad tour__team-info")(
            ...
            tbody(
              info.topPlayers.zipWithIndex.map {
                case (player, index) =>
                  tr(
                    td(index + 1),
                    td(
