...
export default function (data: StudyData, ctrl: AnalyseCtrl, tagTypes: TagTypes, practiceData?: StudyPracticeData, relayData?: RelayData): StudyCtrl {
  ...
  function configureAnalysis() {
    ...
    const computer: boolean = data.chapter.setup.variant.key !== 'russian' && data.chapter.setup.variant.key !== 'brazilian' && !isGamebookPlay() && !!(data.chapter.features.computer || data.chapter.practice);
  ...
  const socketHandlers = {
    ...
    addNode(d) {
      const position = d.p,
        ...
        sticky = d.s;
      setMemberActive(who);
      if (vm.toolTab() == 'multiBoard' || (relay && relay.intro.active) || (multiBoardMenu && multiBoardMenu.open)) multiBoard.addNode(d.p, d.n);
    ...
    clock(d) {
      ...
      if (relay && d.relay) relay.applyChapterRelay(data.chapter, d.relay);
      redraw();
    },
    relay(d) {
    ...
    conceal(d) {
      if (wrongChapter(d)) return;
      data.chapter.conceal = d.displayPly ? d.displayPly : d.ply;
      redraw();
    },
    liking(d) {
  ...
  return {
    ...
    canJumpTo(path: Tree.Path) {
      ...
      const dp = ctrl.tree.lastMainlineNode(path).displayPly;
      return data.chapter.conceal === undefined ||
        isChapterOwner() ||
        treePath.contains(ctrl.path, path) || // can always go back
        (dp ? dp : ctrl.tree.lastMainlineNode(path).ply) <= data.chapter.conceal!;
    },
    onJump() {
